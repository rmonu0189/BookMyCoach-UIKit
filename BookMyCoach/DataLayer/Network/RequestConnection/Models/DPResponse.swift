...
struct DPResponse {
    ...
    init(data: Data?, response: URLResponse?, error: Error?, service: DPRequestProtocol) {
        ...
        self.updateBaseAPIResponse()
        
        // Override user actions
        if let isSuccess = service.success(response: self) { self.isSuccess = isSuccess }
        if let errorResponse = service.error(response: self) { self.error = errorResponse }
        if let message = service.message(response: self) { self.message = message }
    }
    ...
    mutating func updateBaseAPIResponse() {
        if let baseResponse = baseResponseModel(DPBaseAPIResponse.self) {
            self.isSuccess = baseResponse.status
            self.message = baseResponse.message
            if self.isSuccess == false {
                self.error = NSError(domain: "DP_RESPONSE_ERROR", code: self.httpStatusCode?.rawValue ?? 0, userInfo: [NSLocalizedDescriptionKey: self.message ?? ""])
            }
        }
    }
